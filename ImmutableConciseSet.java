...
public class ImmutableConciseSet
{
    ...
    while (iter.hasNext()) {
        ...
        if (ConciseSetUtils.isLiteral(word)) {
          retVal.add(ConciseSetUtils.ALL_ZEROS_LITERAL | ~word);
      	  ...
      	  {
      		  retVal.add(ConciseSetUtils.SEQUENCE_1_FILL | ConciseSetUtils.getSequenceCount(word));
      	  }
      	  else
      	  {
    ...
    if (diff > 0) {
      // first check if the difference can be represented in 31 bits
    	...
    	if (diff <= ConciseSetUtils.MAX_LITERAL_LENGTH) {
            ...
            retVal.add(ConciseSetUtils.SEQUENCE_1_FILL | (endIndexWordCount - wordsWalked));
            retVal.add(ConciseSetUtils.ALL_ONES_LITERAL);
          }
    	}
  ...
  public static int ConvertFill(int word)
  {
	  ...
	  {
		  word &= 0x0fffffff;
		  word |= ConciseSetUtils.SEQUENCE_1_FILL;
		  word += 1;
	  }
	  else
	  {
    ...
    {					
    	...
    	if(ConciseSetUtils.is0_fill(last) || ConciseSetUtils.is1_fill(last))
    	{
    		...
    		if(filltype == fillkind)
    		{
    			if(ConciseSetUtils.getSequenceCount(wordToAdd) + ConciseSetUtils.getSequenceCount(last) <= ConciseSetUtils.MAX_FILL_NUM)
    			{
    				...
    				if(filltype == 1)
    					newWord |= ConciseSetUtils.SEQUENCE_1_FILL;
    	...
    	{				//fill compressed into a f-l-f
    		...
    		else
    		{
    			if(length == 1)
    			{
    				if((ConciseSetUtils.isDirtyByte0Word(last) || ConciseSetUtils.isDirtyByte1Word(last)))	//L-F  in the case that there is only a word in the current sequence
    				{
    				
    					int fillnum = ConciseSetUtils.getSequenceCount(wordToAdd);
    					int filltype = ConciseSetUtils.is0_fill(last)? 0:1;
    					if(fillnum <= ConciseSetUtils.MAX_L_F_NUM)
    					{
    				...
    				{
    					int fillnum = ConciseSetUtils.getSequenceCount(wordToAdd);
    					if(fillnum <= ConciseSetUtils.MAX_NI2L_F_NUM &&(ConciseSetUtils.isDirtyByte0WordBy2(last) || ConciseSetUtils.isDirtyByte1WordBy2(last)))	//NI2L-F  in the case that there is only a word in the current sequence
    					{
    						int filltype = ConciseSetUtils.is0_fill(wordToAdd) ? 0:1;
    						int []literalbyte = new int [2];
    						int []literalPos = new int[2];
    						int pos;
    						int literaltype = ConciseSetUtils.isDirtyByte0WordBy2(last)? 0 : 1;
    						literalbyte = (literaltype == 0)? ConciseSetUtils.getDirty0By2(last) : ConciseSetUtils.getDirty1By2(last);
    						literalPos = (literaltype == 0) ? ConciseSetUtils.getDirtyByte0PosBy2(last) : ConciseSetUtils.getDirtyByte1PosBy2(last);
    						if(literalPos[0] == 0 && literalPos[1] == 1)
    						{
    							pos = 0 ;
    						}
    						else
    						{
    							if(literalPos[0] == 0 && literalPos[1] == 2)
        						{
        							pos = 1 ;
        						}
    							else
    							{
    								if(literalPos[0] == 0 && literalPos[1] == 3)
    	    						{
    	    							pos = 2 ;
    	    						}
    								else
    								{
    									if(literalPos[0] == 1 && literalPos[1] == 2)
    		    						{
    		    							pos = 4 ;
    		    						}
    									else
    									{
    										if(literalPos[0] == 1 && literalPos[1] == 3)
    			    						{
    			    							pos = 5 ;
    			    						}
    										else
    			...
    			else
    			{
    				...
    				{
    					if((ConciseSetUtils.isDirtyByte0Word(last) || ConciseSetUtils.isDirtyByte1Word(last)))	//L-F in the case that there  are more than one word in the current set.
    	    			{
    					...
    					{
    						int fillnum = ConciseSetUtils.getSequenceCount(wordToAdd);
        					if(fillnum <= ConciseSetUtils.MAX_NI2L_F_NUM &&(ConciseSetUtils.isDirtyByte0WordBy2(last) || ConciseSetUtils.isDirtyByte1WordBy2(last)))	//NI2L-F  in the case that there more than a word in the current sequence
        					{
        						int filltype = ConciseSetUtils.is0_fill(wordToAdd) ? 0:1;
        						int []literalbyte = new int [2];
        						int []literalPos = new int[2];
  ...
  public static ImmutableConciseSet doIntersection(Iterator<ImmutableConciseSet> sets)
  {
    ...
    while (!theQ.isEmpty()) {
      ...
      if (currIndex < itr.startIndex) {
        ...
        addAndCompact(retVal, (ConciseSetUtils.SEQUENCE_1_FILL | (itr.startIndex - currIndex)));
        currIndex = itr.startIndex;
      }
    ...
    if (currIndex < wordsWalkedAtSequenceEnd) {
      addAndCompact(retVal, (ConciseSetUtils.SEQUENCE_1_FILL | (wordsWalkedAtSequenceEnd - currIndex)));
    }
        		  ...
        		  else
        		  {
        			  ...
        			  else
        			  {
        				  ...
        				  else
        				  {
        					  ...
        					  {
        						  if(ConciseSetUtils.isL_F(w))
        						  {
        							  int fillnum = ConciseSetUtils.getLFFILLWords(w);
        							  int fillType = (w & 0x02000000) >>> 25;
    	    		    			  int literalnum;
    	    		    			  int literalByte = ConciseSetUtils.getLFLiteralWords(w);
    	    		    			  int literalType = (w & 0x04000000) >>> 26;
        		    				  if(literalType == 0)
        		      			  	  {
        		    					  literalnum = ConciseSetUtils.getLiteralBitCount(literalByte);
        		      			  	  }
        		    				  else
        		    				  {
        		    					  literalnum = ConciseSetUtils.getLiteralBitCount(literalByte) + 23;
        		    				  }
        		    				  retVal += ConciseSetUtils.maxLiteralLengthMultiplication(fillnum * fillType) + literalnum;
        						  }
        						  else
        						  {
        							  if(ConciseSetUtils.isNI2L_F(w))
        							  {
        								  int fillnum = ConciseSetUtils.getNI2LFFILLWords(w);
            							  int fillType = (w & 0x00000080) >>> 7;
        	    		    			  int literalnum;
        	    		    			  int[] literalByte = ConciseSetUtils.getNI2LFLiteralWords(w);
        	    		    			  int literalType = (w & 0x08000000) >>> 27;
            		    				  if(literalType == 0)
            		      			  	  {
            		    					  literalnum = ConciseSetUtils.getLiteralBitCount(literalByte[0]);
            		    					  literalnum += ConciseSetUtils.getLiteralBitCount(literalByte[1]);
            		      			  	  }
            		    				  else
            		    				  {
            		    					  literalnum = ConciseSetUtils.getLiteralBitCount(literalByte[0]);
            		    					  literalnum += ConciseSetUtils.getLiteralBitCount(literalByte[1]) + 15;
            		    				  }
    	  ...
    	  else
    	  {
    		  ...
    		  else
    		  {
    			  ...
    			  else
    			  {
    				  ...
    				  {
    					  int fillnum = ConciseSetUtils.getLFFILLWords(w);
    					  last += ConciseSetUtils.maxLiteralLengthMultiplication(fillnum + 1);
    				  }
    				  else
    				  {
    					  if(ConciseSetUtils.isNI2L_F(w))
    ...
    if (ConciseSetUtils.isLiteral(w)) {
      last -= Integer.numberOfLeadingZeros(ConciseSetUtils.getLiteralBits(w));
    	...
    	else
    	{
    		...
    		{
    			int[] literalByte = ConciseSetUtils.getLFLLiteralWords(w);
    			last -= Integer.numberOfLeadingZeros(ConciseSetUtils.getLiteralBits(literalByte[1]));
    		}
    	}
    }
    return last;
          ...
          else
          {
        	  ...
        	  else
        	  {
        		  if(ConciseSetUtils.isL_F(w))
        		  {
        	          ...
        	          int literalByte = ConciseSetUtils.getLFLiteralWords(w);
        	          int literalPos;
        	          literalPos = (w & 0x01800000) >>> 23;
        	    	  int literal;
        	    	  literalType = (w & 0x04000000) >>> 26;
        	          if(literalType == 0)
            	      {
            	    	  literal = 0x80000000 | (literalByte << (literalPos * 8));
            	      }
        		  ...
        		  else
        		  {
        			  if(ConciseSetUtils.isNI2L_F(w))
        			  {
                		  ...
                		  int filltype = (w & 0x00000080) >>> 7;
            	          int literalType;
            	          int pos = (w & 0x07000000) >>> 24;
            	          int[] literalByte = ConciseSetUtils.getNI2LFLiteralWords(w);
            	          int[] literalPos = new int [2];
            	          switch(pos)
            	          {
            	          case 0: literalPos[0] = 0; literalPos[1] = 1;break;
            	          case 1: literalPos[0] = 0; literalPos[1] = 2;break;
            	          case 2: literalPos[0] = 0; literalPos[1] = 3;break;
            	          case 4: literalPos[0] = 1; literalPos[1] = 2;break;
            	          case 5: literalPos[0] = 1; literalPos[1] = 3;break;
            	          case 6: literalPos[0] = 2; literalPos[1] = 3;
            	          }
            	    	  int literal;
            	    	  literalType = (w & 0x08000000) >>> 27;
	        	    	  literal = (literalByte[0] << (literalPos[0] * 8)) | (literalByte[1] << (literalPos[1] * 8)) 
	        				| 0x80000000;
				         if(literalType == 1)
				         {
				      	   switch(pos)
				      	   {
				      	   case 0: literal |= 0xffff0000;break;
				      	   case 1: literal |= 0xff00ff00;break;
				      	   case 2: literal |= 0x80ffff00;break;
				      	   case 4: literal |= 0xff0000ff;break;
				      	   case 5: literal |= 0x80ff0000;break;
				      	   case 6: literal |= 0x8000ffff;
				      	   }
				         }
  ...
  */
  ...
  private class BitIterator implements IntSet.IntIterator
  {
    ...
    final ConciseSetUtils.LFExpander lfExp;
    final ConciseSetUtils.NI2LFExpander n2lfExp;
    ...
    private BitIterator()
    {
      ...
      lfExp = ConciseSetUtils.newLFExpander();
      n2lfExp = ConciseSetUtils.newNI2LFExpander();
      nextWord();
    }
    ...
    private BitIterator(
        ...
        ConciseSetUtils.LFExpander lfExp,
        ConciseSetUtils.NI2LFExpander n2lfExp,
        int nextIndex,
        int nextOffset
    )
    {
    ...
    public IntSet.IntIterator clone()
    {
      return new BitIterator(
          ...
          (ConciseSetUtils.LFExpander) lfExp.clone(),
          (ConciseSetUtils.NI2LFExpander) n2lfExp.clone(),
          nextIndex,
          nextOffset
      );
    }
      ...
      else
      {
    	  ...
    	  else
    	  {
    		  ...
    		  else
    		  {
    			  ...
    			  {
    				  if(ConciseSetUtils.isNI2L_F(word))
    				  {
    					  exp = n2lfExp;
    				  }
    				  else
    				  {
    					  if(ConciseSetUtils.isL_F(word))
    					  {
    						  exp = lfExp;
    					  }
    				  }
    	  ...
    	  else
    	  {
    		  ...
    		  else
    		  {
    			  ...
    			  else
    			  {
    				  if(ConciseSetUtils.isL_F(word))
    				  {
    					  int fillnum = ConciseSetUtils.getLFFILLWords(word);
    					  nextOffset += ConciseSetUtils.maxLiteralLengthMultiplication(fillnum + 1);
    				  }
    				  else
    				  {
    					  if(ConciseSetUtils.isNI2L_F(word))
    					  {
    						  int fillnum = ConciseSetUtils.getNI2LFFILLWords(word);
    						  nextOffset += ConciseSetUtils.maxLiteralLengthMultiplication(fillnum + 2);
    					  }
    				  }
    			  }
  ...
  public class WordIterator implements Iterator
  {
    ...
    public void advanceTo(int endCount)			//改
    {
      ...
      nextWord = ConciseSetUtils.SEQUENCE_1_FILL | (wordsWalked - endCount);   //需改
      else
      {
    	  ...
    	  else
    	  {
    		  if(ConciseSetUtils.isF1_L_F2(currWord))
    		  {
        	      ...
        	      if(flcount == 1)
    			  {
    				  ...
    				  {
    					  nextWord = ConciseSetUtils.SEQUENCE_1_FILL | (wordsWalked - endCount);
    				  }
    			  }
    			  else
    			  {
    		  ...
    		  else
    		  {
    			  ...
    			  {
    				  if(ConciseSetUtils.isL_F(currWord))
    				  {
    					  int filltype = (currWord & 0x02000000) >>> 25;
            	    	  if(filltype == 0)
        				  {
        					  nextWord = wordsWalked - endCount;
        				  }
    				  ...
    				  {
    					  if(ConciseSetUtils.isNI2L_F(currWord))
    					  {
    						  int filltype = (currWord & 0x00000080) >>> 7;
                	    	  if(filltype == 0)
            				  {
            					  nextWord = wordsWalked - endCount;
            				  }
    ...
    public Integer next()
    {
      if (hasNextWord) {
    	  if(ConciseSetUtils.is0L1_F_0L2(currWord) || ConciseSetUtils.is0L_F_1L(currWord)
				  || ConciseSetUtils.is1L1_F_1L2(currWord) || ConciseSetUtils.is1L_F_0L(currWord)
				  || ConciseSetUtils.isF1_L_F2(currWord) || ConciseSetUtils.isL_F(currWord) 
				  || ConciseSetUtils.isNI2L_F(currWord))
    	  {
    		  hasNextWord = false;
    			...
    			if(ConciseSetUtils.isF1_L_F2(currWord))
    		      {
    		    	  ...
    		    	  words[0] = (filltype[0] << 26) | fillnum[0];
    		    	  words[2] = (filltype[1] << 26) | fillnum[1];
    		    	  if(literaltype == 0)
    		    	  {
    			...
    			else
    			{
    				if(ConciseSetUtils.isL_F(currWord))
    				{
    					int []words = new int[2];
    				      ...
    				      int literalPos;
    				      int literalType;
    				      int literalByte = ConciseSetUtils.getLFLiteralWords(currWord);
    				      fillnum = ConciseSetUtils.getLFFILLWords(currWord);
    				      fillType = (currWord & 0x02000000) >>> 25;
    				      literalPos = (currWord & 0x01800000) >>> 23;
    				      literalType = (currWord & 0x04000000) >>> 26;
    				      if(literalType == 0)
    				      {
    				    	  words[0] = 0x80000000 | (literalByte << (literalPos * 8));
    				      }
    				      else
    				      {
    				    	  words[0] = (literalByte <<(literalPos * 8));
    				    	  switch(literalPos)
    				    	  {
    				    	  case 0: words[0] |= 0xffffff00; break;
    				    	  case 1: words[0] |= 0xffff00ff; break;
    				      ...
    				      if(fillType == 0)
    				      {
    				    	  words[1] = ConciseSetUtils.SEQUENCE_0_FILL | fillnum;
    				      }
    				      else
    				      {
    				    	  words[1] = ConciseSetUtils.SEQUENCE_1_FILL | fillnum;
    				      }
    				      if(flcount == 0)
        		    	  {
        		    		  startIndex = wordsWalked;
    				...
    				else
    				{
    					if(ConciseSetUtils.isNI2L_F(currWord))
    					{
    					      ...
    					      int literalType;
    					      int[] literalPos = new int [2];
    					      int[] literalByte = ConciseSetUtils.getNI2LFLiteralWords(currWord);
    					      int pos = (currWord & 0x07000000) >>> 24;
    					      switch(pos)
    					      {
    					      case 0: literalPos[0] = 0; literalPos[1] = 1;break;
    					      case 1: literalPos[0] = 0; literalPos[1] = 2;break;
    					      ...
    					      literalType = (currWord & 0x08000000) >>> 27;
    					      words[0] = (literalByte[0] << (literalPos[0] * 8)) | (literalByte[1] << (literalPos[1] * 8)) 
    					      				| 0x80000000;
    					      if(literalType == 1)
    					      {
    					    	  switch(pos)
    					    	  {
    					    	  case 0: words[0] |= 0xffff0000;break;
    					    	  case 1: words[0] |= 0xff00ff00;break;
    					    	  case 2: words[0] |= 0x80ffff00;break;
    					    	  case 4: words[0] |= 0xff0000ff;break;
    					    	  case 5: words[0] |= 0x80ff0000;break;
    					    	  case 6: words[0] |= 0x8000ffff;
    					    	  }
    					      }
    					      fillnum = ConciseSetUtils.getNI2LFFILLWords(currWord);
    					      fillType = (currWord & 0x00000080) >>> 7;
    					      if(fillType == 0)
    					      {
    					    	  words[1] = ConciseSetUtils.SEQUENCE_0_FILL | fillnum;
    					      }
